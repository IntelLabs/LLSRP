...
void *decodeSlice(void *Ptr) {
  ...
  while (Running) {
    ...
    DS->Ret = DS->V->decodeFrame(DS->OutData, Mb.SInfo[SPtr].Size, SlicePtr, DS->Num, DS->WindowOpened);
    if (FrameNum != -1)
...
#ifdef MOUSE_TEST
  pthread_create(&ReadMouseThread, NULL, testVirtualMouse, NULL);
  while (Running && Cntr < 100) {
